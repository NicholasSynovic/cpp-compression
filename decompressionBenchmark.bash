...
rm $BZIP2_ARCHIVE_FILEPATH $BZIP2_BACKUP_ARCHIVE_FILEPATH

# Create $BZIP2_ARCHIVE_FILEPATH
src/bin/serial_compression -i $OUTPUT_FILE -o $BZIP2_ARCHIVE_FILEPATH
...
#     cp $BZIP2_BACKUP_ARCHIVE_FILEPATH $BZIP2_ARCHIVE_FILEPATH
#     /usr/bin/time --append --output $OUTPUT_BENCHMARK_DIR/tar.csv --format=$TIME_FORMAT --quiet tar -jxf $INPUT_FILE -C "testData"
#     rm $OUTPUT_FILEPATH
# done
...
for i in $(seq 1 $MAX_ITER);
do
    echo "BZip 2 $ACTION_WORD: $i / $MAX_ITER"
    cp $BACKUP_FILE $INPUT_FILE
    /usr/bin/time --append --output $OUTPUT_BENCHMARK_DIR/bzip2.csv --format=$TIME_FORMAT --quiet bzip2 --decompress --keep --force --quiet $INPUT_FILE
    rm $OUTPUT_FILE
done
...
do
    ...
    cp $BACKUP_FILE $INPUT_FILE
    /usr/bin/time --append --output $OUTPUT_BENCHMARK_DIR/serial_compression.csv --format=$TIME_FORMAT --quiet src/bin/serial_decompression -i $INPUT_FILE -o $OUTPUT_FILE
    rm $OUTPUT_FILE
done
...
do
    ...
    cp $BACKUP_FILE $INPUT_FILE
    /usr/bin/time --append --output $OUTPUT_BENCHMARK_DIR/lbzip2.csv --format=$TIME_FORMAT --quiet lbzip2 --keep --decompress --force --quiet $INPUT_FILE
    rm $OUTPUT_FILE
done
...
do
    ...
    cp $BACKUP_FILE $INPUT_FILE
    /usr/bin/time --append --output $OUTPUT_BENCHMARK_DIR/openmp_compression.csv --format=$TIME_FORMAT --quiet src/bin/openmp_decompression -i $INPUT_FILE -o $OUTPUT_FILE
    rm $OUTPUT_FILE
done
...
# Delete $BZIP2_BACKUP_ARCHIVE_FILEPATH
# Delete $OUTPUT_FILENAME
rm $BZIP2_ARCHIVE_FILEPATH $BZIP2_BACKUP_ARCHIVE_FILEPATH $OUTPUT_FILEPATH
